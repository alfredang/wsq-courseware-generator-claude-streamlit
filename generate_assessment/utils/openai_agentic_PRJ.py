"""
OpenAI SDK-based Project Assessment Generation Module.
...
async def generate_prj_for_lo(client, config, course_title, assessment_duration, learning_outcome, learning_outcome_id, retrieved_content, ability_ids, ability_texts):
    system_message = load_prompt("assessment/project")

    user_task = f"""
    Generate a Project assessment question-answer pair using the following details:
    ...
    try:
        ...
        if qa_result is None or not isinstance(qa_result, dict):
            raise ValueError(f"Failed to parse PRJ response for {learning_outcome_id}")
        return {
        ...
        print(f"Error generating PRJ question for {learning_outcome_id}: {e}")
        return None


async def generate_prj(extracted_data: FacilitatorGuideExtraction, index, model_client, model_choice: str = "DeepSeek-Chat"):
    ...
    for assessment in extracted_data["assessments"]:
        ...
        if "PRJ" in code or "PROJECT" in code:
    ...
    for group in grouped_abilities:
        result = await generate_prj_for_lo(
            client, config,
