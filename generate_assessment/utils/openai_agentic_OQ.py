"""
OpenAI SDK-based Oral Questioning Assessment Generation Module.
...
async def generate_oq_for_lo(client, config, course_title, assessment_duration, learning_outcome, learning_outcome_id, retrieved_content, ability_ids, ability_texts):
    system_message = load_prompt("assessment/oral_questioning")

    user_task = f"""
    Generate a Oral Questioning assessment question-answer pair using the following details:
    ...
    try:
        ...
        if qa_result is None or not isinstance(qa_result, dict):
            raise ValueError(f"Failed to parse OQ response for {learning_outcome_id}")
        return {
        ...
        print(f"Error generating OQ question for {learning_outcome_id}: {e}")
        return None


async def generate_oq(extracted_data: FacilitatorGuideExtraction, index, model_client, model_choice: str = "DeepSeek-Chat"):
    ...
    for assessment in extracted_data["assessments"]:
        ...
        if "OQ" in code or "ORAL QUESTIONING" in code:
    ...
    for group in grouped_abilities:
        result = await generate_oq_for_lo(
            client, config,
